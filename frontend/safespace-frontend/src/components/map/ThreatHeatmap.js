...
const MAJOR_INDIAN_CITIES = {
  'Delhi': { coords: [28.6139, 77.2090], state: 'Delhi', region: 'North' },
  'Mumbai': { coords: [19.0760, 72.8777], state: 'Maharashtra', region: 'West' },
  ...
  'Hyderabad': { coords: [17.3850, 78.4867], state: 'Telangana', region: 'South' },
  ...
  'Ludhiana': { coords: [30.9010, 75.8573], state: 'Punjab', region: 'North' }
};
      ...
      try {
        ...
        if (map && typeof map.getContainer === 'function') {
          ...
          if (container && 
              panes && 
              panes.mapPane && 
              panes.mapPane._leaflet_pos !== undefined) {
  ...
  if (isLoading) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="h-[600px] bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl flex items-center justify-center"
      >
      ...
      <div className="h-[500px] relative">
        {filteredThreats.length > 0 ? (
          ...
          >
                    ...
                    <div className="p-3 max-w-xs">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          threat.threatLevel === 'high' ? 'bg-red-100 text-red-800' :
            ...
            {/* Heat Circles */}
            {showHeatCircles && filteredThreats.map((threat) => (
              <Circle
                ...
                pathOptions={{
                  color: threat.threatLevel === 'high' ? '#DC2626' : 
                         threat.threatLevel === 'medium' ? '#F59E0B' : '#10B981',
                  fillColor: threat.threatLevel === 'high' ? '#DC2626' : 
                            threat.threatLevel === 'medium' ? '#F59E0B' : '#10B981',
                  fillOpacity: 0.1,
                  weight: 2,
                  opacity: 0.4
              ...
              >
                <Popup>
                  <div className="text-center">
                    ...
                    <div className="text-sm text-gray-600">{userLocation.city}, {userLocation.region}</div>
                  </div>
                </Popup>
              </Marker>
            )}
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center space-x-4">
            <span className="text-gray-600">
              🇮🇳 Monitoring {threatStats.total} major Indian cities
            </span>
